...
def SubPixel(grayImg, shape):
    ...
    for y in range(ymax):
        for x in range(xmax):
            I = grayImg[y, x]
            I1 = (grayImg[y, x - 1] + I) / 2 if x > 0 else I
            I2 = (grayImg[y, x + 1] + I) / 2 if x + 1 < xmax else I
            I3 = (grayImg[y - 1, x] + I) / 2 if y > 0 else I
            I4 = (grayImg[y + 1, x] + I) / 2 if y + 1 < ymax else I
    ...
    for y in range(ymax):
        for x in range(xmax):
            for i in range(channels):
                I = imgColor[y, x, i]
                I1 = (imgColor[y, x - 1, i] + I) / 2 if x > 0 else I
                I2 = (imgColor[y, x + 1, i] + I) / 2 if x + 1 < xmax else I
                I3 = (imgColor[y - 1, x, i] + I) / 2 if y > 0 else I
                I4 = (imgColor[y + 1, x, i] + I) / 2 if y + 1 < ymax else I
